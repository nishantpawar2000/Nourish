...
function FoodJournal({ entries, addEntry, settings }) {
  const [text, setText] = useState(''); const [busy, setBusy] = useState(false); const [photo, setPhoto] = useState(null); const cameraRef = useRef(null); const galleryRef = useRef(null);
  const estimate = text => Object.entries(FOOD_LIBRARY).find(([key]) => text.toLowerCase().includes(key))?.[1] || { calories: 360, carbs: 40, protein: 20, fat: 14, title: text.slice(0, 42) || 'Photo meal' };
  async function googleEstimate() { if (!settings.apiKey) return estimate(text); const parts = [{ text: `You are a nutrition logger. Estimate the meal below. Reply ONLY JSON: {"title":"", "calories":0, "carbs":0, "protein":0, "fat":0}. Meal: ${text || 'Identify this food photo.'}` }]; if (photo) parts.push({ inline_data: { mime_type: photo.type, data: photo.data.split(',')[1] } }); const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${settings.model || 'gemini-3-flash-preview'}:generateContent?key=${settings.apiKey}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts }], generationConfig: { responseMimeType: 'application/json', temperature: 0.2 } }) }); if (!response.ok) throw new Error('AI request failed'); const raw = (await response.json()).candidates?.[0]?.content?.parts?.[0]?.text || '{}'; const d = JSON.parse(raw.replace(/```json|```/g, '')); return { title: d.title || text || 'Meal', calories: Number(d.calories) || 0, carbs: Number(d.carbs) || 0, protein: Number(d.protein) || 0, fat: Number(d.fat) || 0 }; }
  async function submit() { if ((!text && !photo) || busy) return; setBusy(true); try { const data = await googleEstimate(); addEntry({ id: crypto.randomUUID(), text: text || 'Photo meal', photo: photo?.data, time: new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }), data }); setText(''); setPhoto(null); } catch { alert('Could not reach Google AI. Check the API key and model in Settings.'); } finally { setBusy(false); } }
  function onPhoto(e) { const file = e.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => setPhoto({ data: reader.result, type: file.type }); reader.readAsDataURL(file); }
  return <section className="journal-screen"><div className="journal-title"><span>FOOD JOURNAL</span><h1>Log your day</h1><p>Describe a meal, add a photo, and let AI handle the details.</p></div>{entries.length === 0 ? <div className="blank-journal"><span><Icon name="magic" size={22}/></span><h2>Your journal is clear</h2><p>No meals have been logged today. Your first entry starts the story.</p></div> : <div className="entry-list">{entries.map((entry, index) => <article className="entry" key={entry.id}><div className={`entry-dot dot-${index % 3}`}>{entry.photo ? <img src={entry.photo} alt=""/> : <Icon name="flame" size={17}/>}</div><div className="entry-main"><div><h3>{entry.data.title}</h3><p>{entry.time}</p></div><div className="entry-energy"><b>{entry.data.calories}</b><span>kcal</span></div></div><div className="entry-macros"><span>Carbs {entry.data.carbs}g</span><span>Protein {entry.data.protein}g</span><span>Fat {entry.data.fat}g</span></div></article>)}</div>}<div className="composer-wrap">{photo && <div className="photo-preview"><img src={photo.data} alt=""/><button onClick={() => setPhoto(null)}><Icon name="close" size={14}/></button></div>}<div className="composer"><button className="photo-button" aria-label="Take a photo" onClick={() => cameraRef.current?.click()}><Icon name="camera" size={20}/></button><button className="photo-button" aria-label="Choose from gallery" onClick={() => galleryRef.current?.click()}><Icon name="image" size={19}/></button><input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === 'Enter' && submit()} placeholder="What did you eat?"/><button className="send-button" disabled={(!text && !photo) || busy} onClick={submit}>{busy ? '…' : <Icon name="send" size={18}/>}</button><input hidden ref={cameraRef} type="file" accept="image/*" capture="environment" onChange={onPhoto}/><input hidden ref={galleryRef} type="file" accept="image/*" onChange={onPhoto}/></div></div></section>;
}
